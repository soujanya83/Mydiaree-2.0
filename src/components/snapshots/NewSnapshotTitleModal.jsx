...
export function NewSnapshotTitleModal({ open, onClose, onSubmit, activeCentreId }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (res.status === "success" || res.status === true) {
        const newId = res.id;
        if (!newId) {
          toast.error("Failed to get snapshot ID from server");
          return;
        }
      ...
      toast.error("An error occurred while creating the snapshot");
    } finally {
      setIsCreating(false);
    }
  };
      ...
      >
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Set up your snapshot with a title, rooms, educators, and children.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm font-semibold text-foreground" htmlFor="snapshot-title">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border bg-muted/20 px-6 py-4">
          <p className="text-xs text-muted-foreground">
            You can add photos, videos, and snapshot details on the next screen.
          </p>
