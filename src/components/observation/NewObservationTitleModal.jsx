...
export function NewObservationTitleModal({ open, onClose, onSubmit, activeCentreId }) {
  ...
  useEffect(() => {
    ...
    const loadRooms = async () => {
      ...
      try {
        const roomsData = await observationService.getRoomsAndStaff(activeCentreId);
        if (roomsData.status) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const payload = {
        center_id: activeCentreId,
        obestitle: title.trim(),
        title: "", // Initial observation content
        notes: "",
        reflection: "",
        child_voice: "",
        future_plan: "",
        implementation: "",
        selected_rooms: selectedRooms, // Array of room IDs
        selected_children: selectedChildren.join(","), // Comma-separated child IDs
        selected_staff: selectedStaff, // Array of staff IDs
      };
      ...
      if (res.status === "success" || res.status === true) {
        const newId = res.id;
        if (!newId) {
          toast.error("Failed to get observation ID from server");
          return;
        }
        toast.success(res.message || "Observation created");
        onSubmit(newId);
      } else {
        toast.error(res.message || "Failed to create observation");
      }
    } catch (err) {
      console.error("Create observation error:", err);
      toast.error("An error occurred while creating the observation");
    } finally {
      setIsCreating(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 border-b border-border px-6 py-5">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1 text-xl font-bold text-foreground">Create New Observation</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Set up your observation with a title, rooms, educators, and children.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                placeholder="e.g., Building bridges together"
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border bg-muted/20 px-6 py-4">
          <p className="text-xs text-muted-foreground">
            You can add assessment milestones, media, and observation notes on the next screen.
          </p>
