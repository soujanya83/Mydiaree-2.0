...
export function QuickActions({ className, gridClassName = "grid-cols-2" }) {
  const [selectedIds, setSelectedIds] = useState(loadQuickActionIds);
  const [customizeOpen, setCustomizeOpen] = useState(false);
  const [draftIds, setDraftIds] = useState(selectedIds);

  const actions = resolveQuickActions(selectedIds);
  ...
  const saveCustomize = () => {
    if (draftIds.length !== MAX_QUICK_ACTIONS) return;
    saveQuickActionIds(draftIds);
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-muted-foreground">{action.subtitle || "Create new"}</p>
              </div>
      ...
      <Dialog open={customizeOpen} onOpenChange={setCustomizeOpen}>
        <DialogContent className="sm:max-w-md max-h-[85vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose exactly {MAX_QUICK_ACTIONS} shortcuts to show on your dashboard.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2 py-2 overflow-y-auto pr-1 flex-1">
            {QUICK_ACTION_POOL.map((action) => {
              const Icon = action.icon;
              const checked = draftIds.includes(action.id);
              const atMax = draftIds.length >= MAX_QUICK_ACTIONS && !checked;
              return (
                <button
                  key={action.id}
                  type="button"
                  ...
                  className={cn(
                    "flex items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition w-full",
                    checked
          ...
          <div className="pt-2 border-t border-border">
            <p className="text-center text-xs text-muted-foreground mb-4">
              Selected {draftIds.length} of {MAX_QUICK_ACTIONS}
            </p>
            <DialogFooter className="gap-2 sm:gap-0">
              ...
              <Button
                ...
                disabled={draftIds.length !== MAX_QUICK_ACTIONS}
              >
                Save
              </Button>
            </DialogFooter>
