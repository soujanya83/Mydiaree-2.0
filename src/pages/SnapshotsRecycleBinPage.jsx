...
export default function SnapshotsRecycleBinPage() {
  ...
  const fetchRecycleBin = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await snapshotService.getRecycleBin(activeCentreId);
      if (res.success) {
        setItems(res.data || []);
      } else {
        toast.error(res.message || "Failed to load snapshots recycle bin");
      }
    } catch (error) {
      console.error("Failed to load snapshots recycle bin:", error);
      toast.error("Error loading snapshots recycle bin");
    } finally {
      setIsLoading(false);
    }
  }, [activeCentreId]);
  ...
  const handleRestore = async () => {
    ...
    try {
      const res = await snapshotService.restoreSnapshot(restoreConfirm.id);
      if (res.success) {
        toast.success(res.message || "Snapshot restored");
        setItems((prev) => prev.filter((item) => item.id !== restoreConfirm.id));
        setRestoreConfirm(null);
      } else {
        toast.error(res.message || "Failed to restore snapshot");
      }
    } catch (error) {
  ...
  const handlePermanentDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await snapshotService.permanentlyDeleteSnapshot(deleteConfirm.id);
      if (res.success) {
        toast.success(res.message || "Snapshot permanently deleted");
        setItems((prev) => prev.filter((item) => item.id !== deleteConfirm.id));
        setDeleteConfirm(null);
      } else {
        toast.error(res.message || "Failed to permanently delete snapshot");
      }
    } catch (error) {
      console.error("Failed to permanently delete snapshot:", error);
      toast.error("Failed to permanently delete snapshot");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              "{stripHtml(restoreConfirm?.title) || "Untitled snapshot"}" will be moved back to the
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this snapshot permanently?</AlertDialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. The snapshot and its media will be permanently removed
              from the system.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function SnapshotRecycleCard({ item, isRestoring, onOpen, onRestore, onDelete }) {
  const images = (item.media || []).filter(isImageMedia);
  const [currentIdx, setCurrentIdx] = useState(0);
  const cover = images[currentIdx];
  const title = stripHtml(item.title) || "Untitled snapshot";
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-auto rounded-md border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-medium text-slate-600 dark:border-slate-800 dark:bg-slate-900/30 dark:text-slate-300">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <span>Children ({item.children_count ?? children.length})</span>
          </div>
          {children.length > 0 ? (
...
function SnapshotGalleryModal({ snap, onClose }) {
  const images = (snap.media || []).filter(isImageMedia);
  const [idx, setIdx] = useState(0);
  const timerRef = useRef(null);
  const title = stripHtml(snap.title) || "Snapshot Gallery";
...
function EmptyState() {
  return (
    <div className="rounded-xl border border-dashed bg-card p-12 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Deleted snapshots will appear here.</p>
    </div>
  );
}
